...
function GraduateGuide() {
  ...
  useEffect(() => {
    Axios.post("/api/graduateGuide/융합에너지신소재공학과/kyoyang").then((res) => {
      if (res.data) {
        setKyoyang(res.data);
      } else {
        alert("failed to");
  ...
  useEffect(() => {
    Axios.post("/api/graduateGuide/융합에너지신소재공학과").then((res) => {
      if (res.data) {
        setMajor(res.data);
      } else {
        alert("failed to");
  ...
  useEffect(() => {
    Axios.post("/api/graduateGuide/융합에너지신소재공학과/grad").then((res) => {
      if (res.data) {
        setGrad(res.data);
      } else {
        alert("failed to");
  ...
  useEffect(() => {
    Axios.post("/api/graduateGuide/융합에너지신소재공학과/sGroup").then((res) => {
      if (res.data) {
        setSGroup(res.data);
      } else {
        alert("failed to");
  ...
  return (
      <>
        <div className="GraduateGuide">
          <h1 className="text-center">융합에너지신소재공학과</h1>
                ...
                <div className="component" id="kyoyang_info">
                  <div className="p-3">
                    ...
                    <p>
                      ...
                      &nbsp; &nbsp; &nbsp; &nbsp; 수치해석및실습, 확률및통계학 <br/><br/>

                      <b>&nbsp; &nbsp; &#10004; 과학 권장</b><br/>
                      &nbsp; &nbsp; &nbsp; &nbsp; 일반물리학및실험1, 일반화학및실험1, 일반화학및실험2 <br/><br/>
